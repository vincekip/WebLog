...
class AddPosts extends Component {
    ...
    componentDidUpdate(props, state){
        if(this.props.match.params.view === 'add' && this.props.admin.posts.filter(p => p.title === this.props.values.title).length > 0){
          const post = this.props.admin.post.filter(p => p.title === this.props.values.title)[0];
        ...
        return (
            <div>
                ...
                <Paper className={classes.leftSide}>
                    ...
                    {this.props.admin.post.PostImages ?
                        this.props.admin.post.PostImages.length > 0 ?
                        <img src={API.makeFileURL(this.props.admin.post.PostImages[0].url, this.props.auth.token)} alt="" className={classes.postImage}/>
                        :null
                    :null}
